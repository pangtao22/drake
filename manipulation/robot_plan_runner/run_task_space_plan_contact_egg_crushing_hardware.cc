...
Eigen::VectorXd CalcInitialJointAngles() {
  ...
  const Eigen::Vector3d p_WQ_start(0.60, -0.38 + 0.1, 0.2);
  const auto R_WL7 = math::RollPitchYawd(0, M_PI / 3, 0).ToRotationMatrix();
...
int run_plan() {
  ...
  vector<Eigen::Quaterniond> quaternions{
      math::RollPitchYawd(0, M_PI / 3, 0).ToQuaternion(),
      math::RollPitchYawd(0, M_PI / 3, 0).ToQuaternion()};

  vector<double> t_knots1{0, 8};
